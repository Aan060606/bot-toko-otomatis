...
// Removed calculateFeeLocally because Saweria automatically calculates the fee
...
bot.action(/^buy_now_(.+)$/, async (ctx) => {
  ...
  try {
    ...
    // Send the base amount to Saweria. Saweria will automatically add the QRIS fee if the account is set to TIPPER pays fee.
    const donation = await createDonation(amount, "pembeli@bot.com", buyerName, "Beli " + productId);
    
    // donation.amount_raw contains the final amount including fee generated by Saweria
    const finalAmount = donation.amount_raw || amount;
